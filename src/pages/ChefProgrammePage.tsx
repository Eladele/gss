              ...
              <tbody>
                {pageRows.map((s, i) => (
                  <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-center text-slate-400">{s.delai}j</td>
        ...
        {filtered.length > PAGE_SIZE && (
          <div className="flex items-center justify-end gap-3 px-5 py-3 border-t border-slate-100 text-sm text-slate-400">
            ...
            <button onClick={() => setPage(p => p - 1)} disabled={page === 0}
              className="px-3 py-1.5 rounded-lg border border-slate-200 disabled:opacity-30 hover:bg-slate-50 text-xs">←</button>
