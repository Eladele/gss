...
export default function ChefSituationsPage() {
  ...
  const mySits: Situation[] = useMemo(() => {
    ...
    if (search) {
      list = list.filter((s) => s.fgp.includes(search) || s.zone.toLowerCase().includes(search.toLowerCase()));
    }
    if (fStatus) {
    ...
    <div className="space-y-5 animate-fade-in max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
        ...
        <p className="text-xs text-slate-400 mt-1.5">
          {ok} situatio{ok !== 1 ? 'ns' : 'n'} terminée{ok !== 1 ? 's' : ''} sur {total}
        </p>
      </div>
                ...
                <div className="px-4 pt-4 pb-2 flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {s.motif && s.motif !== 'oK' && (
                        <span className="text-xs text-slate-400 italic truncate max-w-36" title={s.motif}>
                          {s.motif}
                        </span>
                      )}
                    </div>
              ...
              <tbody>
                {sorted.map((s) => {
                  ...
                  return (
                    <tr
                      key={s.id}
                      className={`border-b border-slate-50 hover:bg-slate-50/60 transition-colors ${s.isUrgent ? 'bg-orange-50/30' : ''}`}
                    >
