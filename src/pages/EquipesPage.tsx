...
function EquipeFormModal({ open, onClose, editing }: { open: boolean; onClose: () => void; editing: Equipe | null }) {
  ...
  useEffect(() => {
    if (editing) {
      setName(editing.name); setLeader(editing.leader); setColor(editing.color);
      setVille(editing.ville ?? 'Nouakchott');
    } else {
      setName(''); setLeader(''); setColor('#1565C0'); setVille('Nouakchott');
    }
  }, [editing, open]);
  ...
  return (
    <Modal open={open} onClose={onClose} title={editing ? `✏️ Modifier ${editing.name}` : '➕ Créer une Équipe'}>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div>
          ...
          {!editing && <p className="text-[10px] text-slate-400 mt-1">Un profil sera automatiquement créé pour le chef (mot de passe "chef2026").</p>}
        </div>
        ...
        <p className="text-[10px] text-slate-400">
          Les membres de l'équipe et le matériel assigné ne se saisissent plus ici : ils sont récupérés automatiquement
          depuis les modules "Employés" et "Matériel" (en fonction de l'équipe RH renseignée sur chaque fiche).
        </p>
        <div className="flex gap-3 justify-end pt-2">
          <Button type="button" variant="outline" onClick={onClose}>Annuler</Button>
          <Button type="submit" disabled={loading}>{loading ? '...' : (editing ? '💾 Enregistrer' : "Créer l'équipe")}</Button>
        </div>
      </form>
    </Modal>
  );
...
export default function EquipesPage() {
  ...
  const openEdit = (eq: Equipe) => { setEditing(eq); setModalOpen(true); };
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex justify-between items-end flex-wrap gap-3">
        ...
        {canManage && (
          <Button icon="➕" onClick={openNew}>Créer une Équipe</Button>
        )}
      </div>
                ...
                <div className="p-5">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-slate-400">{eq.leader} (Chef) · {eq.ville ?? 'Nouakchott'}</div>
                    </div>
                    <div className="text-2xl font-black shrink-0" style={{ color: eq.color }}>{pct}%</div>
                  </div>
                  ...
                  {canManage && (
                    <div className="flex gap-1.5 mb-4">
                      ...
                      {canDelete && (
                        <button onClick={() => { if (confirm(`Supprimer l'équipe ${eq.name} ?`)) removeEquipe(eq.id); }}
                  ...
                  {vehicle && (
                    <div className="mb-4 flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-lg p-2 text-xs">
                      ...
                      <span className={`ml-auto px-2 py-0.5 rounded-full text-[10px] font-bold ${vehicle.statut === 'active' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                        {vehicle.statut}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="mb-4">
                    ...
                    {members.length === 0 ? (
                      ...
                      <div className="space-y-1">
                        ...
                        {techniciens.map(t => (
                          <div key={t.id} className="flex items-center gap-1.5 text-xs bg-slate-50 border border-slate-100 rounded px-2 py-1">
                            <span>🧑‍🔧</span>
                            <span className="font-medium text-slate-700">{t.name}</span>
                            <span className="text-slate-400">Mle {t.mle || '—'} · {t.poste || 'technicien'}</span>
                          </div>
                        ))}
                      </div>
                    )}
