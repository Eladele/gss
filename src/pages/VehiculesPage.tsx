...
const STATUT_LABELS: Record<VehicleStatut, string> = {
  active: 'Active', reserve: 'Réserve', maintenance: 'Maintenance',
};
...
export default function VehiculesPage() {
  ...
  const filtered = useMemo(() => vehicles.filter(v => {
    ...
    if (search && !v.immatriculation.toLowerCase().includes(search.toLowerCase())
      && !v.type.toLowerCase().includes(search.toLowerCase())
      && !(v.equipeNom ?? '').toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  ...
  const openNew = () => { setEditing(null); setForm({ statut: 'active' }); setModalOpen(true); };
  ...
  const save = async () => {
    if (!form.type?.trim() || !form.immatriculation?.trim()) {
      showToast('Type et immatriculation sont obligatoires', 'error'); return;
    }
    if (editing) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <Button icon="➕" onClick={openNew}>Nouveau Véhicule</Button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard value={counts.reserve}     label="En réserve"      icon="🅿️" accent="#E9A93B" />
        ...
        <CardHeader>
          <div className="flex items-center gap-2 flex-wrap w-full">
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 max-w-sm flex-1">
              ...
              <input className="bg-transparent text-sm focus:outline-none flex-1" placeholder="Type, immatriculation, équipe..."
                ...
                <div key={v.id} className="rounded-xl border border-slate-200 overflow-hidden hover:shadow-md transition-shadow bg-white">
                  <div className="p-4">
                    ...
                    <div className="mb-3">
                      ...
                      <Select className="w-full" value={v.statut}
                        onChange={e => changeStatut(v, e.target.value as VehicleStatut)}>
                        <option value="active">Active</option>
                        <option value="reserve">Réserve</option>
                        <option value="maintenance">Maintenance</option>
                      </Select>
                    ...
                    <div className="text-xs text-slate-500 space-y-1 mb-3">
                      <p><span className="text-slate-400">Équipe :</span> {v.equipeNom || '—'}</p>
                      <p><span className="text-slate-400">Chauffeur :</span> {driver?.name || '—'}</p>
                    ...
                    <div className="flex gap-1.5">
                      ...
                      <button onClick={() => { if (confirm(`Supprimer le véhicule ${v.immatriculation} ?`)) removeVehicle(v.id); }}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? `✏️ Modifier ${editing.immatriculation}` : '➕ Nouveau Véhicule'}>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-slate-500 block mb-1.5">Type / modèle *</label>
              <Input value={form.type ?? ''} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} placeholder="L200, GANGO, EXPRESS..." />
            </div>
            <div>
              <label className="text-xs font-semibold text-slate-500 block mb-1.5">Immatriculation *</label>
              <Input value={form.immatriculation ?? ''} onChange={e => setForm(f => ({ ...f, immatriculation: e.target.value }))} placeholder="0000AA00" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-slate-500 block mb-1.5">Statut</label>
              <Select className="w-full" value={form.statut ?? 'active'} onChange={e => setForm(f => ({ ...f, statut: e.target.value as VehicleStatut }))}>
                <option value="active">Active</option>
                <option value="reserve">Réserve</option>
                <option value="maintenance">Maintenance</option>
              </Select>
            ...
            <div>
              ...
              <Input value={form.equipeNom ?? ''} onChange={e => setForm(f => ({ ...f, equipeNom: e.target.value }))} placeholder="ARAFAT, ROSSO..." />
            </div>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 block mb-1.5">Chauffeur</label>
            <Select className="w-full" value={form.chauffeurId ?? ''} onChange={e => setForm(f => ({ ...f, chauffeurId: e.target.value || undefined }))}>
