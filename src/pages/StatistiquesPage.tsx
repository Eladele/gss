...
import {
  statsByEquipe, statsByVille, statsByType, repeatDerangementByClient,
  inPeriod, exportStatsToExcel, type PeriodFilter,
} from '@/utils/stats';
...
const VILLE_COLORS: Record<string, string> = {
  'Nouakchott': '#1565C0', 'Kaédi': '#E9A93B', 'Rosso': '#2E7D32', 'Nouadhibou': '#00838F',
};
...
export default function StatistiquesPage() {
  ...
  const scoped = useMemo(() => situations.filter(s => {
  ...
  const villesActives = new Set(scoped.map(s => {
    const eq = equipes.find(e => e.name.toLowerCase() === s.equipe?.toLowerCase());
  ...
  const trendPoints = useMemo(() => {
    ...
    situations.filter(s => (s.nature ?? 'installation') === nature).forEach(s => {
  ...
  const weekdayData = useMemo(() => {
    ...
    scoped.forEach(s => { if (s.dateDepo) counts[new Date(s.dateDepo).getDay()]++; });
    return WEEKDAYS.map((label, i) => ({ label, value: counts[i] }));
  }, [scoped]);

  // Classement équipes les plus performantes (taux de conformité)
  const leaderboard = useMemo(() => byEquipe
    .filter(e => e.total >= 1)
    .slice()
    .sort((a, b) => b.pctConformite - a.pctConformite || b.total - a.total)
    .slice(0, 5)
    .map(e => ({
      name: e.equipe, sub: `${e.ville} — ${e.total} dossier(s)`,
      value: e.pctConformite, unit: '%', color: getEquipeColor(e.equipe, equipes),
    })), [byEquipe, equipes]);
  ...
  const handleExport = () => {
    exportStatsToExcel({
      ...
      byEquipe, byVille, byType, repeats,
    });
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <Button icon="📤" variant="outline" onClick={handleExport}>Exporter Excel</Button>
      </div>
      ...
      <div className="flex gap-2 bg-slate-100 p-1 rounded-xl w-fit">
        {(['installation', 'derangement'] as SituationNature[]).map(n => (
          ...
          >
            {n === 'installation' ? '🔧 Installation' : '⚠️ Dérangement'}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard value={horsDelai}     label="Hors délai"  icon="⏱️" accent="#C62828" />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle>📊 Équipes les plus actives</CardTitle></CardHeader>
          <div className="p-5">
            {byEquipe.length === 0 ? <EmptyState icon="📭" text="Aucune donnée" /> : (
              <RankedBars data={byEquipe.slice(0, 8).map(e => ({ label: e.equipe, value: e.total, sub: `(${e.pctConformite}%)` }))} color="#1565C0" />
            )}
          </div>
        </Card>
        <Card>
          <CardHeader><CardTitle>📈 Tendance — 30 derniers jours</CardTitle></CardHeader>
          <div className="p-5">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle>🏙️ Répartition par ville</CardTitle></CardHeader>
          <div className="p-5">
            {donutData.length === 0 ? <EmptyState icon="📭" text="Aucune donnée" /> : <DonutChart data={donutData} />}
          </div>
        </Card>
        <Card>
          <CardHeader><CardTitle>📅 Répartition par jour de semaine</CardTitle></CardHeader>
          <div className="p-5">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle>🏆 Équipes les plus performantes (% conformité)</CardTitle></CardHeader>
          <div className="p-5">
            {leaderboard.length === 0 ? <EmptyState icon="🏆" text="Aucune donnée" /> : <Leaderboard items={leaderboard} />}
          </div>
        </Card>
        <Card>
          <CardHeader><CardTitle>🏷️ Par type (CLS, CPL, RLR, CMI, TRL, CST...)</CardTitle></CardHeader>
          <div className="p-5">
            {byType.length === 0 ? <EmptyState icon="📭" text="Aucune donnée" /> : (
      ...
      <Card>
        <CardHeader><CardTitle>🏙️ Détail par ville</CardTitle></CardHeader>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                {['Ville', 'Total', 'Dans délai', 'Hors délai', '% Conf.'].map(h => (
                  <th key={h} className="text-left px-3 py-2 text-xs font-bold text-slate-400 uppercase">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {byVille.length === 0 && <tr><td colSpan={5}><EmptyState icon="📭" text="Aucune donnée" /></td></tr>}
            </tbody>
          </table>
        </div>
      </Card>
                ...
                <tbody>
                  {repeats.map(r => (
                    <tr key={r.fgp} className="border-b border-slate-50">
                      ...
                      <td className="px-3 py-2"><EquipeTag name={r.equipe || '—'} color={getEquipeColor(r.equipe, equipes)} /></td>
                      <td className="px-3 py-2 text-xs text-slate-400 max-w-xs truncate" title={r.motifs.join(' | ')}>{r.motifs.join(' | ') || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
