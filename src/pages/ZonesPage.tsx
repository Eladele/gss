...
export default function ZonesPage() {
  ...
  const handleAddZone = async () => {
    ...
    if (!zone) {
      showToast('Entrez un nom de zone', 'error');
      return;
    }
    if (!selectedTeam) {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">
            {allZones.length} zones · {equipes.length} équipes
          </p>
        </div>
        {isAdmin && (
          <Button
      ...
      <Card>
        <CardHeader>
          <CardTitle> Affectation Zones → Équipes</CardTitle>
        </CardHeader>
        {allZones.length === 0 ? (
          <EmptyState icon="" text="Aucune zone trouvée. Importez des situations ou ajoutez une zone." />
        ) : (
      ...
      {/* ─── Assign Zone Modal */}
      <Modal open={assignOpen} onClose={() => setAssignOpen(false)} title={` Affecter zone ${zoneToAssign}`}>
        <div className="space-y-4">
