...
export default function MaterielsPage() {
    ...
    () =>
      materiels.filter((m) => {
        ...
        if (
          ...
          !(m.equipeNom ?? '').toLowerCase().includes(search.toLowerCase()) &&
  ...
  const save = async () => {
    if (!form.nom?.trim()) {
      showToast('Le nom du matériel est obligatoire', 'error');
      return;
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Suivi du matériel (marteau, pince, cliveuse, source laser, grimpette, power mètre, tournevis...) et de son état
          </p>
        </div>
        <Button onClick={openNew}>Nouveau Matériel</Button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard value={counts.aReparer} label="À réparer" icon="" accent="#E9A93B" />
          ...
          <div className="flex items-center gap-2 flex-wrap w-full">
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 max-w-sm flex-1">
              <input
                ...
                placeholder="Code, matériel, équipe..."
                value={search}
            ...
            <span className="text-xs text-slate-400 ml-auto">{filtered.length} matériel(s)</span>
          </div>
        </CardHeader>
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((m) => (
                  <tr key={m.id} className="hover:bg-slate-50">
                    <td className="px-3 py-2 font-mono text-xs text-slate-500">{m.code || '—'}</td>
                    <td className="px-3 py-2 font-semibold text-slate-800">{m.nom}</td>
                    <td className="px-3 py-2 text-center">{m.quantite}</td>
                    <td className="px-3 py-2">
                      <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${ETAT_STYLES[m.etat]}`}>
                        {ETAT_LABELS[m.etat]}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-slate-600">{m.equipeNom || '— Stock central —'}</td>
                    <td className="px-3 py-2 text-xs text-slate-400 italic max-w-xs truncate" title={m.notes}>
                      {m.notes || '—'}
                    </td>
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      <button onClick={() => openEdit(m)} className="text-blue-600 hover:underline text-xs font-semibold mr-3">
                        Modifier
                      </button>
                      <button
                        onClick={() => {
                          if (confirm(`Supprimer ${m.nom} ?`)) removeMateriel(m.id);
                        }}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? `Modifier ${editing.nom}` : 'Nouveau Matériel'}>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-slate-500 block mb-1.5">Code (inventaire)</label>
              <Input
                value={form.code ?? ''}
          ...
          <div>
            ...
            <Input
              list="materiel-catalogue"
              value={form.nom ?? ''}
              onChange={(e) => setForm((f) => ({ ...f, nom: e.target.value }))}
              placeholder="Marteau, Pince, Cliveuse fibre optique..."
            />
            <datalist id="materiel-catalogue">
              {MATERIEL_CATALOGUE.map((n) => (
          ...
          <div>
            ...
            <Select
              className="w-full"
              value={form.etat ?? 'bon'}
              onChange={(e) => setForm((f) => ({ ...f, etat: e.target.value as EtatMateriel }))}
            >
              <option value="neuf">Neuf</option>
              <option value="bon">Bon état</option>
              <option value="a_reparer">À réparer</option>
          ...
          <div>
            ...
            <Select
              ...
              onChange={(e) => setForm((f) => ({ ...f, equipeNom: e.target.value || undefined }))}
            >
