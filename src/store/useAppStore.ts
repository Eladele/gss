...
import type { User, Situation, Notification, ImportRecord, Equipe, Employee, LeaveRecord, Vehicle, Materiel } from '@/types';
...
export const useAppStore = create<AppState>()(
  ...
  logout: () => set({ user: null, situations: [], importHistory: [], equipes: [], employees: [], leaves: [], vehicles: [], materiels: [] }),
  ...
  markOK: async (fgp) => {
    ...
    set(s => ({
      situations: s.situations.map(sit =>
        sit.fgp === fgp
          ? { ...sit, status: 'ok', updatedAt: new Date().toISOString(), dateClt: sit.dateClt || today }
          : sit
      )
    }));
  ...
  removeMateriel: async (id) => {
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state?.user) {
          state.loadAll();
          if (state.user.role === 'admin') { state.loadEmployees(); state.loadVehicles(); state.loadMateriels(); }
        }
      },
    }
  )
