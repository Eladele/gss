...
export function TypeBadge({ type }: { type: SituationType }) {
  ...
  return <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold ${c.bg} ${c.text}`}>{type}</span>;
}
...
export function Button({ variant = 'primary', size = 'md', icon, children, className = '', ...props }: BtnProps) {
  const base =
    'inline-flex items-center gap-2 font-semibold rounded-lg cursor-pointer transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed';
...
export function CardHeader({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`px-5 py-4 border-b border-slate-100 flex items-center justify-between gap-3 flex-wrap ${className}`}>{children}</div>
  );
}
    ...
    <ToastContext.Provider value={{ showToast }}>
      ...
      <div className="fixed bottom-5 right-5 z-[999] flex flex-col gap-2">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`${bgs[t.type]} text-white px-4 py-3 rounded-xl shadow-xl text-sm font-medium flex items-center gap-2 min-w-64 animate-fade-in`}
          >
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 bg-white ${props.className ?? ''}`}
    />
  );
}
export function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      {...props}
      className={`border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 bg-white ${props.className ?? ''}`}
    />
  );
}
export function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      className={`w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-blue-500 bg-white resize-none ${props.className ?? ''}`}
    />
  );
}
