      ...
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
          ...
          <div
            ...
            style={{ height: `${Math.max(4, (d.value / max) * 110)}px`, background: color, opacity: 0.85 }}
          />
    ...
    <div className="space-y-3">
      {data.map((d, i) => (
        <div key={i} className="flex items-center gap-3">
          <div className="w-28 shrink-0 text-xs font-semibold text-slate-600 truncate text-right" title={d.label}>
            {d.label}
          </div>
          <div className="flex-1 h-4 bg-slate-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full" style={{ width: `${(d.value / max) * 100}%`, background: color }} />
          </div>
          <div className="w-16 shrink-0 text-xs text-slate-700 font-bold">
            {d.value}
            {d.sub ? <span className="text-slate-400 font-normal"> {d.sub}</span> : ''}
          </div>
        </div>
      ))}
    </div>
...
export function Leaderboard({
  ...
  items: { name: string; sub?: string; value: number; unit?: string; color?: string; initials?: string }[];
}) {
  return (
      ...
      {items.map((it, i) => (
        <div key={i} className="flex items-center justify-between py-2.5 border-b border-slate-50 last:border-0">
          <div className="flex items-center gap-3">
            <span
              className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ${MEDAL_BG[i] ?? 'bg-slate-100 text-slate-400'}`}
            >
              {MEDALS[i] ?? i + 1}
            </span>
            <span
