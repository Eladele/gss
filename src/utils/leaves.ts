...
export function exportEmployesPresentsExcel(opts: {
  ...
  const banquesPresentes = [...new Set(present.map(e => e.banque || 'Caisse'))]
    .sort((a, b) => BANQUES_ORDER.indexOf(a) - BANQUES_ORDER.indexOf(b));

  const wb = XLSX.utils.book_new();
  banquesPresentes.forEach((banque, idx) => {
    const rows = present
      .filter(e => (e.banque || 'Caisse') === banque)
      .map(e => ({ ...e, montantSheet: e.montant ?? 0 }));
    const ws = buildVirementSheet({
      banque, employees: rows, ordreNum: nextOrdre(ordreBase, idx), dateStr, motifMois,
    });
    XLSX.utils.book_append_sheet(wb, ws, banque.slice(0, 31));
  });

  // Onglet récapitulatif complet
  const wsAll = XLSX.utils.json_to_sheet(present.map((e, i) => ({
    'Ordre': i + 1, 'Mle': e.mle, 'Nom et prénom': e.name, 'Poste': e.poste || '',
    'Ville': e.ville || '', 'Équipe': e.equipeNom || '', 'Banque': e.banque || 'Caisse',
    'RIB / N° compte': e.rib || '', 'Montant (MRU)': e.montant ?? '',
  })));
