...
export function statsByEquipe(situations: Situation[], equipes: Equipe[], nature?: SituationNature): EquipeStat[] {
  ...
  return Object.entries(byEquipe).map(([equipe, list]) => {
...
export function statsByVille(situations: Situation[], equipes: Equipe[], nature?: SituationNature): VilleStat[] {
  ...
  return Object.entries(byVille).map(([ville, list]) => {
    const horsDelai = list.filter(isHorsDelai).length;
    const total = list.length;
    return {
      ville, total, dansDelai: total - horsDelai, horsDelai,
      pctConformite: total ? Math.round(((total - horsDelai) / total) * 1000) / 10 : 0,
    };
  }).sort((a, b) => b.total - a.total);
}
...
export function statsByType(situations: Situation[], nature?: SituationNature): TypeStat[] {
  ...
  situations
    .filter(s => !nature || (s.nature ?? 'installation') === nature)
    .forEach(s => { (byType[s.type] ??= []).push(s); });
  return Object.entries(byType).map(([type, list]) => {
    const horsDelai = list.filter(isHorsDelai).length;
    const total = list.length;
    return {
      type, total, dansDelai: total - horsDelai, horsDelai,
      pctConformite: total ? Math.round(((total - horsDelai) / total) * 1000) / 10 : 0,
    };
  }).sort((a, b) => b.total - a.total);
}
...
export function exportStatsToExcel(opts: {
  ...
  const wsEquipe = XLSX.utils.json_to_sheet(opts.byEquipe.map(r => ({
    'Équipe': r.equipe, 'Ville': r.ville, 'Total': r.total,
    'Dans délai': r.dansDelai, 'Hors délai': r.horsDelai, '% Conformité': r.pctConformite,
  })));
  XLSX.utils.book_append_sheet(wb, wsEquipe, 'Par équipe');

  const wsVille = XLSX.utils.json_to_sheet(opts.byVille.map(r => ({
    'Ville': r.ville, 'Total': r.total,
    'Dans délai': r.dansDelai, 'Hors délai': r.horsDelai, '% Conformité': r.pctConformite,
  })));
  XLSX.utils.book_append_sheet(wb, wsVille, 'Par ville');

  const wsType = XLSX.utils.json_to_sheet(opts.byType.map(r => ({
    'Type': r.type, 'Total': r.total,
    'Dans délai': r.dansDelai, 'Hors délai': r.horsDelai, '% Conformité': r.pctConformite,
  })));
  XLSX.utils.book_append_sheet(wb, wsType, 'Par type');

  const wsRepeat = XLSX.utils.json_to_sheet(opts.repeats.map(r => ({
    'FGP': r.fgp, 'Nb interventions': r.count, 'Zone': r.zone, 'Équipe': r.equipe,
