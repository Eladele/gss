...
import type { Situation, Equipe, ImportRecord, User, Employee, LeaveRecord, Vehicle, Materiel } from '@/types';
...
export async function createEmployee(emp: Partial<Employee>): Promise<Employee | null> {
  const { data, error } = await supabase.from('employees').insert({
    mle: emp.mle, name: emp.name, poste: emp.poste, banque: emp.banque,
    rib: emp.rib, montant: emp.montant, telephone: emp.telephone,
    nni: emp.nni, ville: emp.ville, equipe_nom: emp.equipeNom,
    date_embauche: emp.dateEmbauche || null, actif: emp.actif ?? true,
  }).select().single();
  if (error) { console.error('createEmployee:', error); return null; }
...
export async function updateEmployee(id: string, emp: Partial<Employee>): Promise<void> {
  const { error } = await supabase.from('employees').update({
    ...
    rib: emp.rib, montant: emp.montant, telephone: emp.telephone,
    nni: emp.nni, ville: emp.ville, equipe_nom: emp.equipeNom,
    date_embauche: emp.dateEmbauche || null, actif: emp.actif,
  }).eq('id', id);
...
export async function createLeave(leave: Partial<LeaveRecord>): Promise<LeaveRecord | null> {
  const { data, error } = await supabase.from('employee_leaves').insert({
    employee_id: leave.employeeId, type: leave.type, date_debut: leave.dateDebut,
    date_fin: leave.dateFin, jours: leave.jours, motif: leave.motif ?? '',
    created_by: leave.createdBy ?? '',
  }).select().single();
  if (error) { console.error('createLeave:', error); return null; }
...
export async function createVehicle(v: Partial<Vehicle>): Promise<Vehicle | null> {
  const { data, error } = await supabase.from('vehicles').insert({
    type: v.type, immatriculation: v.immatriculation, statut: v.statut ?? 'active',
    statut_depuis: v.statutDepuis ?? new Date().toISOString(),
    equipe_nom: v.equipeNom, chauffeur_id: v.chauffeurId || null, notes: v.notes ?? '',
  }).select().single();
  if (error) { console.error('createVehicle:', error); return null; }
  return mapVehicle(data);
}

export async function updateVehicle(id: string, v: Partial<Vehicle>): Promise<void> {
  const payload: Record<string, any> = {
    type: v.type, immatriculation: v.immatriculation, statut: v.statut,
    equipe_nom: v.equipeNom, chauffeur_id: v.chauffeurId || null, notes: v.notes,
  };
...
export async function fetchMateriels(): Promise<Materiel[]> {
  ...
  if (error) { console.error('fetchMateriels:', error); return []; }
  return (data ?? []).map(mapMateriel);
}

export async function createMateriel(m: Partial<Materiel>): Promise<Materiel | null> {
  const { data, error } = await supabase.from('materiels').insert({
    nom: m.nom, equipe_nom: m.equipeNom || null, quantite: m.quantite ?? 1,
    etat: m.etat ?? 'bon', notes: m.notes ?? '',
  }).select().single();
  if (error) { console.error('createMateriel:', error); return null; }
  return mapMateriel(data);
}

export async function updateMateriel(id: string, m: Partial<Materiel>): Promise<void> {
  const { error } = await supabase.from('materiels').update({
    nom: m.nom, equipe_nom: m.equipeNom || null, quantite: m.quantite,
    etat: m.etat, notes: m.notes,
  }).eq('id', id);
